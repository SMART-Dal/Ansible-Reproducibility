...
def write_results_csv(output_tasks, new_output_tasks, input_file, output_directory):
    try:
        ...
        output_file_v1 = os.path.join(
            output_dir, f"{os.path.basename(input_file)}_smells_v1_{timestamp}.csv"
        )
        output_file_v2 = os.path.join(
            output_dir, f"{os.path.basename(input_file)}_smells_v2_{timestamp}.csv"
        )
        ...
        with (
            open(output_file_v1, "a", newline="") as file_v1,
            open(output_file_v2, "a", newline="") as file_v2,
        ):
