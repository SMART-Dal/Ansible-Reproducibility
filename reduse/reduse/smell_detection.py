...
def detect_fixed_versions(package_list):
    fixed_version_pattern = re.compile(r"^[\w-]+(?:=[\d.]+(?:-[\w\d.]+)?)?$")
    fixed_versions = {}

    for package in package_list:
        if fixed_version_pattern.match(package):
            # Split package and version
            parts = package.split("=")
            if len(parts) == 2:
                package_name = parts[0].strip()
                version = parts[1].strip()
                fixed_versions[package_name] = version
...
def is_newer_version_available(package_name, installed_version, package_manager):
    try:
        ...
        available_version = subprocess.check_output(
            [manager_module.find_program(package_manager), "info", package_name]
        ).decode("utf-8")
...
def check_task_for_broken_dependency(task):
    try:
        ...
        package_installers_keys_to_check = [
            ...
            "ansible.builtin.dnf_key",
            "ansible.builtin.pacman_key",
        ...
        def check_repository_assumption(task_part):
            if has_package_repository_assumption(
                task, task_part, url_download_components
            ):
                url = (
                    task_part.get("url")
                    or task_part.get("repo")
                    or task_part.get("mirrorlist")
                    or task_part.get("baseurl")
                )
                if url and not check_url_validity(url):
                    messages.append(
                        f"Task assumes a package repository with an invalid URL: {url}"
                    )
    ...
    package_installers = [
        {
            "name": "apt",
            "latest_state": "latest",
            "update_actions": ["upgrade", "update_cache"],
        },
        {
            "name": "yum",
            "latest_state": "latest",
        ...
        for t in task:
            for installer in package_installers:
                if installer["name"] in t:
                    if is_latest_install(installer, task[t]) or is_update_cache(
                        task[t]
                    ):
                        messages.append(
                            f"Task uses {installer['name']} to install the latest packages."
                        )
                    else:
                        if "version" in task[t]:
                            if is_newer_version_available(
                                task["name"], task["version"], installer["name"]
                            ):
                                messages.append(
                                    "Task is downloading an outdated version of the package"
                                )
                        messages.append(
...
def is_latest_install(installer, task_attributes):
    return (
        "state" in task_attributes
        and task_attributes["state"] == installer["latest_state"]
    )
    ...
    try:
        for t in task:
            ...
            if is_firewall_task(t) and "state" not in task[t]:
                messages.append(
                    "Task changes the state of the firewall without checking."
                )
...
def is_file_task(task):
    return (
        ...
        or "copy" in task
        or "lineinfile" in task
        or "mount" in task
        or "blockinfile" in task
    )
    ...
    try:
        for t in task:
            for pm in package_managers:
                if pm in t:
                    if "version" in task[t] or detect_fixed_versions(task[t]):
                        messages.append(
                            f"Task uses {pm} to install a specific version of a package."
                        )
                    else:
                        messages.append(
                            f"Task uses {pm} to install a latest version which could get incompatible."
                        )
        if messages:
            return "\n".join(messages)
        else:
        ...
        print(
            "Error occurred while checking task for version-specific packages:", str(e)
        )
        return "Error"
    ...
    try:
        for t in task:
            for component in ["command", "shell", "raw"]:
                if component in t:
                    messages.append(
                        check_hardware_or_software_components(
                            task[t],
                            ["lspci", "lshw"],
                    ...
                    messages.append(
                        check_hardware_or_software_components(
                            task[t],
                            ["cpufrequtils", "sysctl", "cpufreq-info", "cpu_affinity"],
        ...
        print(
            "Error occurred while checking task for hardware-specific commands:", str(e)
        )
        return "Error"
...
def check_task_for_software_specific_commands(task):
    ...
    try:
            ...
            if "command" in t or "shell" in t or "raw" in t:
                for command in software_commands:
                    if command in task[t]:
                        messages.append(
                            f"Task uses a {command} command that may not be portable."
                        )
                        break
            else:
                messages.append(
                    check_hardware_or_software_components(
                        t, software_commands, "task uses a software specific command."
                    )
                )
        if messages:
            return "\n".join(messages)
        ...
        print(
            "Error occurred while checking task for software-specific commands:", str(e)
        )
        return "Error"
    ...
    try:
        for t in task:
            ...
            if has_firewall_assumption(task, t):
                messages.append(
                    "Task assumes the firewall and changes the state without checking."
                )

            if has_dns_assumption(task, t):
                messages.append(
                    "Task assumes that the system is using a resolv.conf file to manage DNS settings."
                )

            if has_ethernet_assumption(task, t):
                messages.append(
                    "Task changes ethernet interfaces settings without checking the state."
                )

            if has_ntp_assumption(task, t):
                messages.append(
                    "Task assumes the system is using the ntp service to manage time settings and the provided NTP servers."
                )

            if has_ssh_assumption(task, t):
                messages.append(
                    "Task assumes that the system is using SSH without checking the state."
                )
...
def has_environment_assumption(task, t):
    return (
        ("vars" in t or "include_vars" in t or "include_tasks" in t or "when" in t)
        and ("ansible_distribution" in str(task[t]))
        or (
            "selinux" in t
            or "ansible.windows.win_optional_feature" in t
            or "ansible.windows.win_package" in t
        )
    )
...
def has_os_family_assumption(task, t):
    return (
        ("vars" in t or "include_vars" in t or "include_tasks" in t or "when" in t)
        and ("ansible_os_family" in str(task[t]))
        or (
            "selinux" in t
            or "ansible.windows.win_optional_feature" in t
            or "ansible.windows.win_package" in t
...
def has_firewall_assumption(task, t):
    return (
        "ansible.posix.firewalld" in t
        or "community.general.ufw" in t
        or "firewall_rule" in t
    ) and ("state" not in task[t])
...
def has_package_repository_assumption(task, t, key_download_components):
    return any(key_checker in str(t) for key_checker in key_download_components) and (
        "url" in str(task[t]) or "repo" in task[t] or "repository" in task[t]
    )
    ...
    try:
        for t in task:
            if "msg" in t or "failed_when" in t:
                if (
                    "dependencies are missing" in task[t]
                    or "dependency not found" in task[t]
                ):
                ...
                src_message = check_path(
                    task,
                    t,
                    "src",
                    "Task is using absolute path for source",
                    "Task is using relative path for source",
                )
                path_message = check_path(
                    task,
                    t,
                    "path",
...
def create_task_smells(task_name, smell_name_description):
    task_smells = {
        "Task name": task_name,
        "Unguarded operations": smell_name_description["Unguarded operations"],
        "Incompatible version dependency": smell_name_description[
        ...
        "Assumption about environment": smell_name_description[
            "Assumption about Environment"
        ],
        "Hardware specific commands": smell_name_description[
...
def perform_smell_detection_for_task(task):
    ...
    try:
        smell_name_description["Unguarded operations"] = check_task_for_idempotency(
            task=task
        )
        smell_name_description["Incompatible version dependency"] = (
            check_task_for_version_specific_package(task=task)
        )
        ...
        smell_name_description["Hardware Specific Commands"] = (
            check_task_for_hardware_specific_commands(task=task)
        )
        assumption = (
            check_task_for_environment_assumptions(task=task)
            + " "
            + check_task_for_software_specific_commands(task=task)
        )
        smell_name_description["Assumption about Environment"] = assumption
        smell_name_description["Broken Dependency Chain"] = (
            check_task_for_broken_dependency(task=task)
        )
    except Exception as e:
