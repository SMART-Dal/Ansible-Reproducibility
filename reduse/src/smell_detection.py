...
def create_task_smells(task_name, smell_name_description):
    task_smells = {
        ...
        'Incompatible version dependency': smell_name_description['Incompatible version dependency'],
...
def perform_smell_detection_for_task(task):
    ...
    try:
        smell_name_description['Unguarded operations'] = check_task_for_idempotency(task=task)
        smell_name_description['Incompatible version dependency'] = check_task_for_version_specific_package(task=task)
