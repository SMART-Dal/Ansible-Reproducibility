...
if __name__ == "__main__":
    ...
    correlation_coefficient, p_value = spearmanr(get_column_values(csv_file, 'Assumption about environment'),
                                                get_column_values(csv_file, 'Broken Dependency'))
    print(f"spearman Correlation Coefficient: {correlation_coefficient}")
    print(f"P-value: {p_value}")

    shapiro_stat, p_value = shapiro(get_column_values(csv_file, 'Hardware specific commands'))
